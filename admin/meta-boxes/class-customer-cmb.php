...
class Customer_CMB
{
	...
	public function details_html($post)
	{
		...
		<table class="form-table">
			<tr>
				...
				<td>
					<input type="email" name="<?=$this->post_type?>_email" id="<?=$this->post_type?>_email" value="<?=$email?>">
				</td>
			</tr>		
		</table>			
		<?php
	...
	{
		if (array_key_exists("{$this->post_type}_email", $_POST)) {
			update_post_meta(
				$post_id,
				"_{$this->post_type}_email",
				$_POST["{$this->post_type}_email"]
			);
		}
	}
